...
params = {'beta (borough)': beta_borough,
          'mean latent period': mean_latent_period,
...
# compute the terms from the effectives
def home_term(beta_nta,_nta_,_effective_dict_):
    return beta_nta * home_eff(_nta_,_effective_dict_)[0]/home_eff(_nta_,_effective_dict_)[1]

# average of the beta values for home and for borough multiplied by the effective i over n total that we're exposed to
def inborough_commuter_term(beta_nta,_nta_,_effective_dict_):
    return np.average(np.array([beta_nta,beta_borough])) * (inborough_eff(_nta_,_effective_dict_)[0] + home_eff(_nta_,_effective_dict_)[0]) / (inborough_eff(_nta_,_effective_dict_)[1] + home_eff(_nta_,_effective_dict_)[1])

# average of the beta values multiplied by the populations exposed to
def car_commuter_term(beta_nta,_nta_,_effective_dict_):
    return np.average(np.array([beta_nta,beta_borough])) * (commuter_eff(_nta_,_effective_dict_)[0] + home_eff(_nta_,_effective_dict_)[0]) / (commuter_eff(_nta_,_effective_dict_)[1] + home_eff(_nta_,_effective_dict_)[1])

# average of the beta values multiplied by the populations exposed to
def metro_commuter_term(beta_nta,beta_metro,_nta_,_effective_dict_):
    return np.average(np.array([beta_nta,beta_borough,beta_metro])) * (commuter_eff(_nta_,_effective_dict_)[0] + home_eff(_nta_,_effective_dict_)[0] + metro_eff(_nta_,_effective_dict_)[0]) / (commuter_eff(_nta_,_effective_dict_)[1] + home_eff(_nta_,_effective_dict_)[1] + metro_eff(_nta_,_effective_dict_)[1])

# defined according to array (S,E,Is,Ia,R,D)
def matrices(beta_metro,beta_nta,_effective_dict_,_nta_):

    home_matrix = np.array([[-(home_term(beta_nta,_nta_,_effective_dict_)),0,0,0,0,0],
                            [home_term(beta_nta,_nta_,_effective_dict_),-mean_latent_period**(-1),0,0,0,0],
    ...
    subway_matrix = np.array([[-(metro_commuter_term(beta_nta,beta_metro,_nta_,_effective_dict_)),0,0,0,0,0],
                            [metro_commuter_term(beta_nta,beta_metro,_nta_,_effective_dict_),-mean_latent_period**(-1),0,0,0,0],
                            [0,proportion_symptomatic*mean_latent_period**(-1),-mean_infectious_period**(-1)*(proportion_severe_cases+1),0,0,0],
                            [0,(1-proportion_symptomatic)*mean_latent_period**(-1),0,-mean_infectious_period**(-1),0,0],
                            [0,0,mean_infectious_period**(-1),mean_infectious_period**(-1),0,0],
                            [0,0,mean_infectious_period**(-1)*proportion_severe_cases,0,0,0]])

    car_matrix = np.array([[-(car_commuter_term(beta_nta,_nta_,_effective_dict_)),0,0,0,0,0],
                            [car_commuter_term(beta_nta,_nta_,_effective_dict_),-mean_latent_period**(-1),0,0,0,0],
                            [0,proportion_symptomatic*mean_latent_period**(-1),-mean_infectious_period**(-1)*(proportion_severe_cases+1),0,0,0],
                            [0,(1-proportion_symptomatic)*mean_latent_period**(-1),0,-mean_infectious_period**(-1),0,0],
                            [0,0,mean_infectious_period**(-1),mean_infectious_period**(-1),0,0],
                            [0,0,mean_infectious_period**(-1)*proportion_severe_cases,0,0,0]])

    inborough_matrix = np.array([[-(inborough_commuter_term(beta_nta,_nta_,_effective_dict_)),0,0,0,0,0],
                            [inborough_commuter_term(beta_nta,_nta_,_effective_dict_),-mean_latent_period**(-1),0,0,0,0],
                            [0,proportion_symptomatic*mean_latent_period**(-1),-mean_infectious_period**(-1)*(proportion_severe_cases+1),0,0,0],
                            [0,(1-proportion_symptomatic)*mean_latent_period**(-1),0,-mean_infectious_period**(-1),0,0],
                            [0,0,mean_infectious_period**(-1),mean_infectious_period**(-1),0,0],
                            [0,0,mean_infectious_period**(-1)*proportion_severe_cases,0,0,0]])
...
def main(time,beta_metro,beta_nta_factor):
    ...
    for i in range(4):
        ...
        print('Starter NTA '+str(i)+' is '+nonzero_metro_ntas[starter_nta])

    # write a file containing the paramaters
    params['beta (metro)'] = beta_metro
    ...
    par.write('NTA beta factor after gov restrictions: '+ str(beta_nta_factor))
    par.close()
    # data output file
...
#### two week lag
        ...
        if step==33:
            ...
            # beta_nta is drastically reduced because of no school etc.
            beta_nta = 0.5*beta_nta_factor
...
###################################################################
        ...
        for nta in ntas:
            # create their matrix
            m = matrices(beta_metro,beta_nta,effectives,nta)
            for i in range(len(categories)):
