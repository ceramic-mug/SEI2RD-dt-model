...
def metro_term(beta_metro,_nta_,_effective_dict_):
...
def matrices(beta_metro,_effective_dict_,_nta_):
    ...
    subway_matrix = np.array([[-(home_term(_nta_,_effective_dict_)+commuter_term(_nta_,_effective_dict_)+metro_term(beta_metro,_nta_,_effective_dict_)),0,0,0,0,0],
                            [home_term(_nta_,_effective_dict_)+commuter_term(_nta_,_effective_dict_)+metro_term(beta_metro,_nta_,_effective_dict_),-mean_latent_period**(-1),0,0,0,0],
                            [0,proportion_symptomatic*mean_latent_period**(-1),-mean_infectious_period**(-1)*(proportion_severe_cases+1),0,0,0],
                            [0,(1-proportion_symptomatic)*mean_latent_period**(-1),0,-mean_infectious_period**(-1),0,0],
                            [0,0,mean_infectious_period**(-1),mean_infectious_period**(-1),0,0],
                            [0,0,mean_infectious_period**(-1)*proportion_severe_cases,0,0,0]])
...
def main(time,beta_metro):
    ...
    # seed the model with a random infected person
...
###################################################################
        ...
        for nta in ntas:
            # create their matrix
            m = matrices(beta_metro,effectives,nta)
            for i in range(len(categories)):
